...
namespace KumariCinemas
{
    public partial class UserTickets : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e) { if (!IsPostBack) LoadCustomerDropdown(); }
        ...
        private void LoadCustomerDropdown()
        {
            ...
            {
                conn.Open();
                var da = new OracleDataAdapter("SELECT USER_ID, USER_NAME FROM CUSTOMER ORDER BY USER_ID", conn);
                var dt = new DataTable(); da.Fill(dt);
                ddlCustomer.DataSource = dt; ddlCustomer.DataTextField = "USER_NAME"; ddlCustomer.DataValueField = "USER_ID"; ddlCustomer.DataBind();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            using (var conn = new OracleConnection(connectionString))
            {
                conn.Open();
                // User info
                var cmdU = new OracleCommand("SELECT * FROM CUSTOMER WHERE USER_ID=:custid", conn);
                ...
                string sql = @"SELECT T.TICKET_ID, M.MOVIE_TITLE, H.HALL_NAME, TH.THEATER_NAME,
                               T.SEAT_NO, T.TICKET_PRICE, T.TICKET_STATUS, T.BOOKING_TIME, S.SHOW_DATE
                ...
                var dt = new DataTable(); da.Fill(dt);
                gvUserTickets.DataSource = dt; gvUserTickets.DataBind();
            }
        }
    }
}